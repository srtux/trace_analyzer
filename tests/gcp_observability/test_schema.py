...
import pytest
from gcp_observability.schema import (
...
def test_trace_comparison_report():
    report = TraceComparisonReport(
        baseline_summary=TraceSummary(trace_id="1", span_count=1, total_duration_ms=10.0),
        ...
        latency_findings=[
            LatencyDiff(
                span_name="db", baseline_ms=5.0, target_ms=10.0,
                diff_ms=5.0, diff_percent=100.0, severity=Severity.HIGH
            )
        ]
    )
...
def test_sub_agent_reports():
    ...
    lat_report = LatencyAnalysisReport(
        baseline_trace_id="1", target_trace_id="2", overall_diff_ms=10.0,
    ...
    err_report = ErrorAnalysisReport(
        baseline_error_count=0, target_error_count=1, net_change=1,
    ...
    struct_report = StructureAnalysisReport(
        baseline_span_count=10, baseline_depth=3,
        target_span_count=12, target_depth=4,
...
def test_statistical_reports():
    stats = StatisticalAnalysisReport(
        latency_distribution=LatencyDistribution(
            sample_size=100, mean_ms=50, median_ms=50, p90_ms=80, p95_ms=90,
            p99_ms=100, std_dev_ms=10, coefficient_of_variation=0.2
        ),
...
def test_service_impact_report():
    impact = ServiceImpactReport(
        total_services_analyzed=5, impacted_services_count=1,
        blast_radius_assessment="low"
    )
    assert impact.total_services_analyzed == 5

def test_observability_schemas():
    log = LogEntry(timestamp="2023-01-01T00:00:00Z", severity="INFO", payload="test", resource={})
    assert log.payload == "test"

    ts = TimeSeries(metric={}, resource={}, points=[TimeSeriesPoint(timestamp="now", value=1.0)])
