...
from gcp_observability.tools.analysis.trace.statistical_analysis import (
...
@patch("gcp_observability.tools.analysis.trace.statistical_analysis.fetch_trace_data")
...
@patch("gcp_observability.tools.analysis.trace.statistical_analysis.fetch_trace_data")
@patch("gcp_observability.tools.analysis.trace.statistical_analysis.analyze_critical_path")
@patch("gcp_observability.tools.analysis.trace.analysis.build_call_graph")
def test_perform_causal_analysis_success(mock_build_graph, mock_analyze_critical, mock_fetch):
    # Baseline: Span A takes 10ms
    # Target: Span A takes 100ms
    baseline_data = {
        "spans": [
            {
                "span_id": "b1", "name": "spanA", "duration_ms": 10,
    ...
    target_data = {
        "spans": [
            {
                "span_id": "t1", "name": "spanA", "duration_ms": 100,
                "start_time": "2024-01-01T00:00:00Z",
...
@patch("gcp_observability.tools.analysis.trace.statistical_analysis._fetch_traces_parallel")
def test_analyze_trace_patterns_mocked_fetch(mock_fetch_parallel):
    t1 = {
        "trace_id": "t1", "duration_ms": 200, "spans": [
            {"name": "spanA", "duration_ms": 150}
        ]
    }
    t2 = {
        "trace_id": "t2", "duration_ms": 200, "spans": [
            {"name": "spanA", "duration_ms": 155}
        ]
    }
    t3 = {
        "trace_id": "t3", "duration_ms": 200, "spans": [
            {"name": "spanA", "duration_ms": 145}
        ]
    }
...
@patch("gcp_observability.tools.analysis.trace.statistical_analysis.compute_latency_statistics")
@patch("gcp_observability.tools.analysis.trace.statistical_analysis.fetch_trace_data")
def test_detect_latency_anomalies_success(mock_fetch, mock_compute):
    ...
    target_data = {
        "duration_ms": 200,
        "spans": [
            {"name": "spanA", "duration_ms": 60, "start_time": "2024-01-01T00:00:00Z", "end_time": "2024-01-01T00:00:00.060Z"}
        ]
    }
