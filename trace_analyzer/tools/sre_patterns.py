...
- Lock contention: Spans waiting on locks/mutexes
- Cold start latency: Unusually slow first requests
- Thundering herd: Many parallel requests to same resource
"""
...
TIMEOUT_INDICATORS = ["timeout", "deadline", "exceeded", "timed out", "context deadline"]
CONNECTION_INDICATORS = ["connection", "pool", "acquire", "checkout", "wait"]
LOCK_INDICATORS = ["lock", "mutex", "semaphore", "sync", "wait", "acquire"]
COLD_START_INDICATORS = ["init", "startup", "cold", "warmup", "bootstrap"]
...
def _get_span_duration(span: dict) -> float | None:
...
def detect_retry_storm(
    ...
    for span in spans:
        name = span.get("name", "")
        spans_by_name[name].append(span)
    ...
    for name, span_list in spans_by_name.items():
        ...
        if len(span_list) >= threshold or is_retry_span:
            # Sort by start time
            sorted_spans = sorted(
                span_list, key=lambda s: _parse_timestamp(s.get("start_time", "")) or 0
            )
...
def detect_cascading_timeout(
    ...
    for span in spans:
        ...
        duration = _get_span_duration(span) or 0

        if is_timeout or duration >= timeout_threshold_ms:
            timeout_spans.append(
                {
                    **_extract_span_info(span),
                    "is_explicit_timeout": is_timeout,
                    "start_ms": _parse_timestamp(span.get("start_time", "")),
                }
            )
    ...
    if len(timeout_spans) >= 2:
        ...
        for timeout_span in timeout_spans:
            ...
            while current_id:
                parent_timeout = next(
                    (t for t in timeout_spans if t.get("span_id") == current_id), None
                )
                if parent_timeout:
    ...
    unique_chains = []
    for chain in sorted(cascade_chains, key=lambda c: c["chain_length"], reverse=True):
        affected = set(chain["affected_spans"])
        is_subset = any(affected <= set(c["affected_spans"]) for c in unique_chains)
        if not is_subset:
...
def detect_connection_pool_issues(
            ...
            pool_issues.append(
                {
                    ...
                    "severity": (
                        ...
                        else "medium" if duration >= wait_threshold_ms * 2 else "low"
                    ),
                }
            )
    ...
    return {
        ...
        "has_pool_exhaustion": len(pool_issues) > 0 and total_wait >= wait_threshold_ms * 3,
        "recommendation": (
